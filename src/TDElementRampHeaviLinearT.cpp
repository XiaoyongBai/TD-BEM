...
TDElementRampHeaviLinearT::TDElementRampHeaviLinearT(int type):
		TDElementT(type)
{
	//do nothing
}

TDElementRampHeaviLinearT::~TDElementRampHeaviLinearT()
{
}


void TDElementRampHeaviLinearT::FormGH_Regular()
{
	...
	double* DT_Kelvin_1_T=new double[9];
	double* DT_Kelvin_2_T=new double[9];
	...
	for(int q_1=0; q_1<fNQ; q_1++)
	{
		for (int q_2=0; q_2<fNQ; q_2++)
		{
			...
			fGreenFunction->Compute(5);

			double* DT_Kelvin_1, *DT_Kelvin_2;
			fGreenFunction->GetKelvinRampedLinear(&DT_Kelvin_1, &DT_Kelvin_2);
			...
			MathOperationT::transMatrix(3,3, DT_Kelvin_1, DT_Kelvin_1_T);
			MathOperationT::transMatrix(3,3, DT_Kelvin_2, DT_Kelvin_2_T);
			MathOperationT::VecPlus(3*3, 1, fC_1, -w*J, DT_Kelvin_1_T, fC_1);
			MathOperationT::VecPlus(3*3, 1, fC_2, -w*J, DT_Kelvin_2_T, fC_2);
	...
	delete[] direction;
	delete[] matrix_temp;
	delete[] DT_Kelvin_1_T;
	delete[] DT_Kelvin_2_T;
 }


void TDElementRampHeaviLinearT::FormGH_Singular(int singular)
{
	...
	if (fType==1)
	{
		FormGH_Singular_1(singular, fShape, fGE_1, fGE_2, fHE_1, fHE_2);
	}
	else if(fType==2)
	{
		SubElement(singular);
		...
		for(int se_i=0; se_i<fSubNum; se_i++)
		{
			...
			FormGH_Singular_1(fSubCID[se_i], fSubShape, GE_1, GE_2, HE_1, HE_2);
...
void TDElementRampHeaviLinearT::SubElement(int singular)
{
...
void TDElementRampHeaviLinearT::FormGH_Singular_1(int singular, ShapeFunctionT* Shape,
								   double* GE_1, double* GE_2, double* HE_1, double* HE_2)
{
	...
	for(int q_1=0; q_1<fNQ; q_1++)
	{
		for (int q_2=0; q_2<fNQ; q_2++)
		{
			...
			//MathOperationT::PrintMatrix(3, DT_Kelvin_1, "DT_Kelvin_1");
			//MathOperationT::PrintMatrix(3, DT_Kelvin_2, "DT_Kelvin_2");
			//MathOperationT::PrintMatrix(3, DU_Stokes_1, "fDU_Stokes_1");
			//MathOperationT::PrintMatrix(3, DU_Stokes_2, "fDU_Stokes_2");
			...
			for(int i=0; i<3;i++)
			{
				for(int j=0; j<3; j++)
				{
					HE_1[i*sd*ennd+sd*singular+j] -= w*J_tri*J*DT_Kelvin_1[j*sd+i];
					HE_2[i*sd*ennd+sd*singular+j] -= w*J_tri*J*DT_Kelvin_2[j*sd+i];
				}
			}
...
void TDElementRampHeaviLinearT::AssembleSubElement(const int* ids, const double* GE_1, const double* GE_2,
